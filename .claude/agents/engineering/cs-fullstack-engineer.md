...
2. Read the story, acceptance criteria, architecture constraints, dependencies, and test notes in full.
3. Skip the seven-question grill when the planning package already answers the relevant stack and product constraints.
4. Use `bmad-dev-story` for implementation and update only the story sections that workflow permits.
5. Route frontend-only or backend-only sub-work to `cs-frontend-engineer` or `cs-backend-engineer` only when it reduces risk or preserves context.
6. Run story-specific verification and record tests.
7. Run `bmad-code-review` and `cs-karpathy-reviewer`; fix review follow-ups before done.
8. Return a digest under 200 words: story path, changed files, tests run, review result, unresolved risks, and next story recommendation.
