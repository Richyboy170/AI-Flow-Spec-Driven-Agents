...
- Is the OAuth `redirect_uri` an exact match against a registered allowlist (no wildcard hosts, no open redirect)? A loose or attacker-influenced `redirect_uri` lets the post-auth code/token — and the user's session — land on the wrong origin or device.
...
- **Invoke via:** `/ship` (parallel fan-out alongside `code-reviewer` and `test-engineer`), any future `/audit` command, or `Agent({subagent_type:"security-auditor",...})` dispatched by `cs-engineering-lead` as part of Phase 4 story review fan-out.
...
- **Authorized orchestrator exception:** `cs-engineering-lead` may dispatch this agent as an independent sibling reviewer in Phase 4 story loop fan-out when a story touches auth, APIs, data protection, AI/LLM features, or third-party integrations. Critical or High findings block the story done gate. This does not authorize peer reviewers to invoke each other.
