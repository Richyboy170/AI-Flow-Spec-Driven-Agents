...
- **Invoke via:** `/webperf` (dedicated performance audit command), or `Agent({subagent_type:"web-performance-auditor",...})` dispatched by `cs-engineering-lead` for frontend/UI stories in Phase 4 story review fan-out. Not included in `/ship` fan-out — performance audits apply to web applications only, not to utility libraries or CLI tools, so adding it to a global pre-launch fan-out would create noise in non-web projects.
...
- **Authorized orchestrator exception:** `cs-engineering-lead` may dispatch this agent as an independent sibling reviewer in Phase 4 story loop fan-out for frontend and UI stories where LCP, INP, or CLS paths are affected. This does not authorize peer reviewers to invoke each other.
