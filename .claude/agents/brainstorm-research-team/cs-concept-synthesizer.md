...
Read the upstream inputs first: the lead's `/tmp/idea-grill-<date>.md`, the brainstorming session file, the market + tech research digests, and (for UI-bearing work) the visual report plus `{project_root}/assets/visual-manifest.json` and root asset directory.
...
1. Read the inputs: locked problem + ICP, the chosen direction/wedge, the market digest, the tech-feasibility digest, visual report/manifest paths when applicable, and the riskiest assumption (Q7).
...
3. **Product brief** — problem, target user, value proposition, MVP scope + explicit non-goals, success metrics, and the riskiest-assumption test from Q7. For UI-bearing work, add a `Visual Evidence Inputs` section containing the visual report, root `{project_root}/assets/` directory, manifest path, named-brand coverage totals and gaps, evidence-backed theme/palette, use/avoid rules, rights summary, and unresolved user approvals. Label it research input, not final art direction; `bmad-ux` owns approval and `DESIGN.md`.
...
ALWAYS return a compact digest with: the one-line pitch, PRFAQ headline, value proposition, riskiest assumption + test, input gaps, PRFAQ/product-brief paths, and visual report/manifest paths when applicable.
