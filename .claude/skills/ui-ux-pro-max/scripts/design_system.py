...
from core import search, DATA_DIR, _load_csv, CSV_CONFIG
...
# ---- House default: Flat Design ----------------------------------------------
# This skill defaults to flat design (Swiss / Metro lineage): solid colors, no
# shadows / gradients / blur / skeuomorphism, clean sans-serif type, ample
# whitespace. Effects-heavy styles are opt-in only — they apply when the user
# explicitly names one in the query. This prevents the engine from defaulting to
# glossy "digital" styles (Glassmorphism, Neumorphism, Liquid Glass, Aurora UI,
# Motion-Driven, etc.) just because a product category maps to one.

# Exact style name (must match a "Style Category" row in styles.csv).
DEFAULT_FLAT_STYLE = "Flat Design"

# Style families that are already flat — if the reasoning picks one of these,
# keep it (no override needed).
FLAT_FAMILY = ("flat design", "minimalism", "swiss", "accessible", "inclusive")

# Query substrings that count as an explicit request for an effects-heavy style.
# When any of these is present we respect the user's choice and skip the
# flat-design override.
EXPLICIT_STYLE_KEYWORDS = (
    "glassmorphism", "glass", "frosted",
    "neumorphism", "neumorphic", "soft ui",
    "claymorphism", "clay",
    "skeuomorph", "skeuomorphism",
    "brutalism", "brutalist",
    "liquid glass",
    "aurora",
    "retro", "retro-futurism", "synthwave", "vaporwave", "cyberpunk",
    "3d", "hyperrealism",
    "motion-driven", "motion driven",
    "dark mode", "oled",
    "gradient", "mesh gradient", "iridescent",
)


def _explicit_style_request(query: str) -> bool:
    """True if the query explicitly names an effects-heavy style to opt into."""
    q = (query or "").lower()
    return any(kw in q for kw in EXPLICIT_STYLE_KEYWORDS)


def _load_style_row(style_name: str) -> dict:
    """Load a single style row from styles.csv by exact Style Category name.

    Returns the row mapped to the same output columns the search engine uses,
    so it is a drop-in replacement for a `search` style result.
    """
    filepath = DATA_DIR / CSV_CONFIG["style"]["file"]
    output_cols = CSV_CONFIG["style"]["output_cols"]
    if not filepath.exists():
        return {}
    for row in _load_csv(filepath):
        if row.get("Style Category", "").strip().lower() == style_name.lower():
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # House default: flatten the selection unless the user explicitly asked
        # for an effects-heavy style. The reasoning rules lead many product
        # categories with glossy styles (Glassmorphism, Liquid Glass, Aurora UI,
        # Motion-Driven, etc.); override those to Flat Design so the default
        # aesthetic is flat (Swiss / Metro), not "digital".
        selected_style_name = best_style.get("Style Category", "").lower()
        already_flat = any(fam in selected_style_name for fam in FLAT_FAMILY)
        if not already_flat and not _explicit_style_request(query):
            flat_row = _load_style_row(DEFAULT_FLAT_STYLE)
            if flat_row:
                best_style = flat_row
...
def format_master_md(design_system: dict) -> str:
    ...
    # Elevation (Flat Design — separation by border / solid fill, not shadow)
    lines.append("### Elevation (Flat)")
    lines.append("")
    lines.append("Flat design separates surfaces with **solid color and 1px hairline borders**, not shadows. Avoid drop shadows, gradients, and blur.")
    lines.append("")
    lines.append("| Token | Value | Usage |")
    lines.append("|-------|-------|-------|")
    lines.append("| `--border-hairline` | `1px solid var(--color-border)` | Cards, inputs, dividers |")
    lines.append("| `--radius-sm` | `2px` | Buttons, inputs, tags |")
    lines.append("| `--radius-md` | `4px` | Cards, modals |")
    lines.append("| `--shadow` | `none` | No shadows in flat design |")
    lines.append("")
    ...
    lines.append("")
    lines.append("```css")
    lines.append("/* Primary Button — flat: solid fill, no shadow, no lift */")
    ...
    lines.append("  box-shadow: none;")
    lines.append("  font-weight: 600;")
    lines.append("  transition: background-color 150ms ease, opacity 150ms ease;")
    lines.append("  cursor: pointer;")
    lines.append("}")
    lines.append("")
    lines.append(".btn-primary:hover {")
    lines.append("  /* Flat: shift color/opacity, never translate or add shadow */")
    lines.append("  opacity: 0.9;")
    lines.append("}")
    lines.append("")
    lines.append("/* Secondary Button — flat outline */")
    ...
    lines.append("  box-shadow: none;")
    lines.append("  font-weight: 600;")
    lines.append("  transition: background-color 150ms ease, color 150ms ease;")
    lines.append("  cursor: pointer;")
    lines.append("}")
    lines.append("```")
    lines.append("")
    ...
    lines.append("```css")
    lines.append(".card {")
    lines.append(f"  background: {colors.get('card', colors.get('background', '#FFFFFF'))};")
    lines.append("  border: 1px solid var(--color-border);")
    lines.append("  border-radius: 4px;")
    lines.append("  padding: 24px;")
    lines.append("  box-shadow: none; /* Flat: separate with border, not shadow */")
    lines.append("  transition: background-color 150ms ease, border-color 150ms ease;")
    lines.append("  cursor: pointer;")
    lines.append("}")
    lines.append("")
    lines.append(".card:hover {")
    lines.append("  /* Flat: shift border/background, never lift or shadow */")
    ...
    lines.append("  box-shadow: none;")
    lines.append("  transition: border-color 150ms ease;")
    lines.append("}")
    lines.append("")
    lines.append(".input:focus {")
    lines.append(f"  border-color: {colors.get('primary', '#2563EB')};")
    lines.append("  /* Flat: a solid 2px outline keeps focus visible (WCAG) without a soft glow */")
    lines.append(f"  outline: 2px solid {colors.get('primary', '#2563EB')};")
    lines.append("  outline-offset: 1px;")
    lines.append("}")
    lines.append("```")
    lines.append("")
    ...
    lines.append("  /* Flat: solid scrim for legibility — no backdrop blur */")
    ...
    lines.append("")
    lines.append("- ❌ **Drop shadows / elevation** — Flat design separates surfaces with solid color + 1px borders, not shadows")
    lines.append("- ❌ **Gradients** — Use solid, bold colors (gradients read as glossy/skeuomorphic)")
    lines.append("- ❌ **Backdrop blur / glass effects** — No `backdrop-filter`; use a solid scrim instead")
    lines.append("- ❌ **Large border-radius (8px+)** — Keep corners sharp or near-sharp (0–4px)")
    lines.append("- ❌ **translateY/scale lift on hover** — Shift color or opacity instead")
